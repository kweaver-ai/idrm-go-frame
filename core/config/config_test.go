...
func (p *testJSONSource) Load() ([]*sources.KeyValue, error) {
	kv := &sources.KeyValue{
		Key:    "json",
		Value:  []byte(p.data),
		Format: "json",
...
func (p *testJSONSource) Watch() (sources.Watcher, error) {
...
func newTestWatcher(sig, err chan struct{}) sources.Watcher {
	return &testWatcher{sig: sig, err: err, exit: make(chan struct{})}
}

func (w *testWatcher) Next() ([]*sources.KeyValue, error) {
	select {
	case <-w.sig:
		return nil, nil
	case <-w.err:
...
func TestConfig(t *testing.T) {
	...
	opts := options{
		sources:  []sources.Source{jSource},
