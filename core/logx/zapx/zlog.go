...
func CustomLoads(skip int, cs LogConfigs) {
	CustomLoad(skip, cs.Logs...)
}

// 根据配置，创建新的logger对象，增加一次skip
func CustomLoad(skip int, cs ...Options) {
	hasDefault := false
	if skip < 0 {
		skip = 0
	}
	for _, c := range cs {
		...
		loggerMap[c.Name] = c.ZapLoggerAddSkip(skip)
		if c.Default {
			hasDefault = true
			std = loggerMap[c.Name]
		}
